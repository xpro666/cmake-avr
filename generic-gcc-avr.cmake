...
function(add_avr_executable EXECUTABLE_NAME)
  ...
  else (NOT CONFIG_LOAD_ADDR)
    set_target_properties(
        ${elf_file}
        PROPERTIES
        COMPILE_FLAGS "-mmcu=${AVR_MCU}"
        LINK_FLAGS "-mmcu=${AVR_MCU} -Wl,--gc-sections -mrelax -Wl,-Map,${map_file} -Wl,--section-start=.text=${CONFIG_LOAD_ADDR}"
    )
  endif (NOT CONFIG_LOAD_ADDR)
...
function(avr_generate_fixed_targets)
  ...
  # set fuses
  if (AVR_E_FUSE AND AVR_LOCK_FUSE)
    add_custom_target(
        set_fuses
        ...
        -U lock:w:${AVR_LOCK_FUSE}:m
        COMMENT "Setup: High Fuse: ${AVR_H_FUSE} Low Fuse: ${AVR_L_FUSE} E Fuse: ${AVR_E_FUSE} Lock: ${AVR_LOCK_FUSE}"
    )
  elseif (AVR_E_FUSE)
    add_custom_target(
        set_fuses
  ...
  else (AVR_E_FUSE AND AVR_LOCK_FUSE)
    add_custom_target(
        set_fuses
        ${AVR_UPLOADTOOL} ${AVR_UPLOADTOOL_BASE_OPTIONS} -P ${AVR_UPLOADTOOL_PORT}
        -U lfuse:w:${AVR_L_FUSE}:m
  ...
  endif (AVR_E_FUSE AND AVR_LOCK_FUSE)
